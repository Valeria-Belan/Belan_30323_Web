...
namespace Belan_30323.UI.TagHelpers
{
    ...
    public class PagerTagHelper(LinkGenerator linkGenerator, IHttpContextAccessor httpContextAccessor) : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            output.TagName = "div";
            output.AddClass("row", HtmlEncoder.Default);
            output.AddClass("centered-pagination", HtmlEncoder.Default);  // Добавляем новый класс для центрирования
            ...
            #region Кнопка предыдущей страницы
            ul.InnerHtml.AppendHtml( CreateListItem(Category, Prev, "<span ariahidden=\"true\">&laquo;</span>"));
            #endregion Кнопка предыдущей страницы
            ...
            #region Кнопка следующей страницы
            ul.InnerHtml.AppendHtml( CreateListItem(Category, Next, "<span ariahidden=\"true\">&raquo;</span>"));
            #endregion Кнопка следующей страницы
